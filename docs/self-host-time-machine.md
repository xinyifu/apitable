...
- 点击操作记录项：保持现有历史预览行为。
- 点击记录 chip：打开记录详情。
- 点击 chip 内的标题或 `recordId` 文本：不再设置独立点击行为，避免在同一个小标签内误触。
- 当前记录已删除时，先点击对应操作记录进入历史预览；如果该记录在预览版本中存在，再点击 chip 打开预览版本中的记录详情。
- 预览状态需要提供明确退出入口，避免用户不知道如何回到当前数据。
...
- 已删除记录在对应历史预览中支持打开记录详情。
- recordId 文本不再单独复制，统一由整个 chip 打开详情。
...
- 修正时光机预览：在克隆快照上应用回滚 actions 后再渲染 `previewDatasheet`。
- 时光机预览底部浮层增加“退出预览”按钮。
- 预览状态下按 Esc 退出预览。
- 底部退出后同步取消右侧操作记录选中态。
...
- 点击 chip 内的 `recordId` 文本，与点击 chip 其他区域行为一致，都是打开记录详情。
- 点击已删除记录 chip 且未处于对应历史预览时，显示“先点击这条操作记录进入历史预览”的反馈提示。
- 点击包含删除记录的操作记录后进入历史预览，再点击该记录 chip，会打开预览版本中的记录详情。
- 历史预览中的表格内容应反映对应版本的历史快照，而不是只显示当前表并改一个预览提示。
- 点击 chip 不影响整条操作记录点击预览历史版本的能力。
- 历史预览底部浮层右上角显示关闭图标。
- 点击关闭图标后回到当前数据，右侧操作记录选中态取消。
- 历史预览状态下按 Esc 可以退出预览。
- 控制台不出现时光机组件新增的运行时错误。
